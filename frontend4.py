...
def appendCad():
        ...
        for i in range(k):
            ...
            # elif kontainer == 4:
            # itembaris=k/kontainer
            # if itembaris<maksJob:


            # if i<=6 :
            print((int(i/7)*0.190*b.sH)+(0.175*b.sH))
            panjangButton=(wContainer-((0.01*b.sW)+((7+1)*0.01*b.sW)))/7
            offsetH=0.170*b.sH
            offsetW=0.163*b.sW
            btnTask[i].place(x=(i%7)* (panjangButton+(0.01*b.sW))+offsetW,y=((int(i/7)*0.190*b.sH)+offsetH),width=panjangButton,height=0.158*b.sH) 
            # elif i>6 and i <=13:
            #     c=i-7
            #     #print(c)
            #     btnTask[i].place(x=c*(((pnjangBtn-((7+1)*0.01*b.sW))/7)+0.01*b.sW)+((0.163*b.sW)),y=0.347*b.sH,width=(pnjangBtn-((0.01*b.sW)+((7+1)*0.01*b.sW)))/7,height=0.158*b.sH)
