...
class Beranda:
    ...
    def showLayar(self):
       ...
       # self.btn.place(x=200,y=200,width= 50,height=50)
        ...
        self.pegawai=["Imam","Aji","Dika"]
        print(len(self.pegawai))
        self.labelPegawai=[0 for z in range(5)]
        for j in range(len(self.pegawai)):
            self.labelPegawai[j]= Label(self.frame,text = self.pegawai[j],bg="WHITE",fg='#1687A7',font=10)
            self.labelPegawai[j].place(x=0.085*self.sW,y=j*((0.078*self.sH)+0.09*self.sH)+(0.243*self.sH),width=0.056*self.sW,height=0.078*self.sH,anchor=W)


        self.pnjangBtn=0.843 * self.sW
        self.k=5
        self.btnTask =[0 for x in range(88)]
        for i in range(self.k):
            self.btnTask[i] = Button(self.frame,image = self.gambar2,text="hallo "+str(i),command = self.tertekan)
            self.btnTask[i].place(x=i*(((self.pnjangBtn-((self.k+1)*0.01*self.sW))/self.k)+0.01*self.sW)+((0.163*self.sW)),y=0.175*self.sH,width=(self.pnjangBtn-((0.01*self.sW)+((self.k+1)*0.01*self.sW)))/self.k,height=0.158*self.sH) 
        # for i in range(4):
        #     for j in range(4):
    ...
    def tertekan(self):
        ...
        self.centangButton=Button(self.frame2,image = self.centangPhoto,borderwidth=0,bg="WHITE",command=self.centangPressed)
